...
- Deterministic scenario metrics should come after the `Event` -> `Processor`
  -> `Ready` boundary can be driven by replayable input traces.
...
later sync/store/simulation work justify them:
